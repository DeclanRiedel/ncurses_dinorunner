      ...
      if (a.shape[ay][ax] != ' ') {
        for (int by = 0; by < b.shape.size(); by++) {
          for (int bx = 0; bx < b.shape[by].length(); bx++) {
            if (b.shape[by][bx] != ' ' && a.x + ax == b.x + bx &&
                a.y + ay == b.y + by) {
              return true;
            }
          }
...
void createObject(ObjectType type) {
  ...
  case ObjectType::Obstacle: {
    ...
    gameObjects.push_back({WIDTH - 1, GROUND - height + 1, shape,
                           ObjectType::Obstacle, isCactus ? 1 : 2});
    break;
  }
  case ObjectType::Bush: {
    ...
    gameObjects.push_back({WIDTH - 1,
                           GROUND + yOffset,
                           {std::string(1, bushShape)},
                           ObjectType::Bush,
                           color});
    break;
  }
  case ObjectType::Star: {
    gameObjects.push_back({rand() % WIDTH,
                           rand() % (GROUND - 5),
                           {std::string(1, STAR)},
                           ObjectType::Star,
                           0});
...
void updateGame() {
  ...
  gameObjects.erase(std::remove_if(gameObjects.begin(), gameObjects.end(),
                                   [](const GameObject &obj) {
                                     return obj.type != ObjectType::Dino &&
                                            obj.x + obj.shape[0].length() < 0;
                                   }),
                    gameObjects.end());
  ...
  double jumpOffset =
      isJumping ? sin(jumpProgress / JUMP_DURATION * PI) * JUMP_HEIGHT : 0;
...
int main() {
  ...
  while (!gameOver) {
    ...
    if (nextObstacleDistance <= 0) {
      ...
      nextObstacleDistance =
          rand() % (MAX_OBSTACLE_DISTANCE - MIN_OBSTACLE_DISTANCE + 1) +
  ...
  mvprintw(startY + DEFEAT_BANNER.size() + 1, WIDTH / 2 - 7, "Final Score: %d",
           score);
  mvprintw(startY + DEFEAT_BANNER.size() + 3, WIDTH / 2 - 10,
           "Press 'q' to quit");
